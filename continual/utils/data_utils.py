...
def compute_data_moments(loader):
    data_list = list()
    for i, (data, _) in enumerate(loader):
        data_list.append(data)
    whole_data = torch.cat(data_list, dim=0)
    whole_data = whole_data.reshape((whole_data.shape[0], -1))
    inputs_mean = torch.mean(whole_data, dim=0)
    inputs_var = torch.diag(torch.var(whole_data, dim=0) + 1e-8)
    inputs_cov = torch.cov(whole_data.t()) + torch.eye(
        whole_data.shape[1], whole_data.shape[1]) * 1e-4
    return inputs_mean, inputs_var, inputs_cov, data_list[0]

def compute_k_moments(loader, label_set):
    # label set is a list of lists containing the labels in each of k clusters
    all_mean, all_var, all_cov, all_val = [], [], [], []
    for label in label_set:
        data_list = list()
        for i, (data, y) in enumerate(loader):
            data_list.append(data[torch.argwhere(y == label[0]).flatten()])
            data_list.append(data[torch.argwhere(y == label[1]).flatten()])
        whole_data = torch.cat(data_list, dim=0)
        whole_data = whole_data.reshape((whole_data.shape[0], -1))
        inputs_mean = torch.mean(whole_data, dim=0)
        inputs_var = torch.diag(torch.var(whole_data, dim=0) + 1e-8)
        inputs_cov = torch.cov(whole_data.t()) + torch.eye(
            whole_data.shape[1], whole_data.shape[1]) * 1e-4
        all_mean.append(inputs_mean)
        all_var.append(inputs_var)
        all_cov.append(inputs_cov)
        all_val.append(data_list[0])
    return all_mean, all_var, all_cov, all_val

def compute_classwise_moments(loader, label_set):
    all_mean, all_var, all_cov, all_val = [], [], [], []
    for label in label_set:
        data_list = list()
        for i, (data, y) in enumerate(loader):
            data_list.append(data[torch.argwhere(y == label).flatten()])
        whole_data = torch.cat(data_list, dim=0)
        whole_data = whole_data.reshape((whole_data.shape[0], -1))
        inputs_mean = torch.mean(whole_data, dim=0)
        inputs_var = torch.diag(torch.var(whole_data, dim=0) + 1e-8)
...
class PermutedMnistGenerator():
    ...
    def __init__(self, max_iter=10, random_seed=0):
        ...
        try:
            ...
            f = gzip.open(
                '/scratch/ssd001/home/huang/git_code/fsd/codebase/data/mnist.pkl.gz',
                'rb')
        ...
        self.class_list = range(
            10
        )  # List of unique classes being considered, in the order they appear
    ...
    def next_task(self):
        if self.cur_iter >= self.max_iter:
            ...
            return TensorDataset(next_x_train, next_y_train), TensorDataset(
                next_x_test, next_y_test)
    ...
    def next_task(self):
        if self.cur_iter >= self.max_iter:
            ...
            for class_index in range(np.size(self.sets[self.cur_iter])):
                ...
                train_id = np.where(self.train_label == self.sets[
                    self.cur_iter][class_index])[0]
                ...
                if class_index == 0:
                    ...
                    next_y_train = np.concatenate(
                        (next_y_train, next_y_train_interm), axis=0)

                # Repeat above process for test inputs
                test_id = np.where(self.test_label == self.sets[self.cur_iter]
                                   [class_index])[0]
                if class_index == 0:
                    next_x_test = self.X_test[test_id]
                else:
                    next_x_test = np.vstack(
                        (next_x_test, self.X_test[test_id]))
                ...
                if class_index == 0:
                    ...
                    next_y_test = np.concatenate(
                        (next_y_test, next_y_test_interm), axis=0)
            ...
            next_y_test = torch.from_numpy(next_y_test)
            return TensorDataset(next_x_train, next_y_train), TensorDataset(
                next_x_test, next_y_test), self.sets[self.cur_iter - 1]
...
class SplitCIFAR100:
    ...
    def get_dims(self):
        ...
        return len(self.train_dataset
                   ) / self.nr_classes_per_task, self.nr_classes_per_task

    def next_task(self):
        if self.cur_iter >= self.max_iter:
            raise Exception('Number of tasks exceeded!')
        else:
            train_dataset = SplitDataSet(self.train_dataset, self.cur_iter,
                                         self.nr_classes,
                                         self.nr_classes_per_task)
            val_dataset = SplitDataSet(self.val_dataset, self.cur_iter,
                                       self.nr_classes,
                                       self.nr_classes_per_task)
...
class SplitDataSet(Dataset):
    ...
    def __init__(self, dataset, cur_iter, nr_classes, nr_classes_per_task):
        ...
        task_idx = torch.nonzero(
            torch.from_numpy(
                np.isin(
                    targets,
                    self.classes[nr_classes_per_task *
                                 self.cur_iter:nr_classes_per_task *
                                 self.cur_iter + nr_classes_per_task])))
