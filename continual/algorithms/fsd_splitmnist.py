...
from continual.utils.data_utils import compute_data_moments, compute_classwise_moments, SplitMnistGenerator
from continual.utils.model_utils import parameters_to_vector, SplitMLP
...
class FSDSplitMnist(object):
    ...
    def compute_fsd(self, params0, params1, data_moments, mus, model, fsd_type,
                    val_batch, label_set, optpar_dict, fisher_dict):
        if fsd_type == 'ewc':
    ...
    def compute_fsd_single(self, params0, params1, data_moments, mus, model,
        ...
        if fsd_type == 'None':
            ...
            fsd = lin_act(
                ...
                MEAN,
                COV,
                self.hparams.train.num_samples,
                self.output_metric,
    ...
    def run_splitmnist(self, fsd_type):
        ...
        layer_size = [
            ...
            self.hparams.model.hidden_size, num_tasks * num_classes_per_task
        ]
        ...
        val_batches = []
        all_mus = []
        fisher_dict, optpar_dict = [dict() for _ in range(num_tasks)
                                    ], [dict() for _ in range(num_tasks)]
                    ...
                    if tid > 0:
                        ...
                        for prev_tid in range(tid):
                            fsd += self.compute_fsd(
                                ...
                                label_sets[prev_tid], optpar_dict[prev_tid],
                                fisher_dict[prev_tid])
                        fsd /= tid
