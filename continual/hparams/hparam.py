...
class Hparam(object):
    ...
    def to_dict(self):
        ...
        res_dict = {}
        for key in self.__dict__:
            is_list, istup = isinstance(self.__dict__[key], list), isinstance(
                self.__dict__[key], tuple)
            if isinstance(self.__dict__[key], Hparam):
                res_dict[key] = self.__dict__[key].to_dict()
            elif is_list or istup:
                temp = []
                for item in self.__dict__[key]:
                    if isinstance(item, Hparam):
                        item = item.to_dict()
                    temp.append(item)
                if istup:
                    temp = tuple(temp)
    ...
    def to_dict_str(self):
        ...
        for key in self.__dict__:
            is_list, istup = isinstance(self.__dict__[key], list), isinstance(
                self.__dict__[key], tuple)
            if isinstance(self.__dict__[key], Hparam):
                res_dict[key] = self.__dict__[key].to_dict_str()
            elif is_list or istup:
                temp = []
                for item in self.__dict__[key]:
                    if isinstance(item, Hparam):
                        item = item.to_dict_str()
                    temp.append(item)
                if istup:
                    temp = tuple(temp)
                res_dict[key] = str(temp)
            else:
                res_dict[key] = str(self.__dict__[key])
        return res_dict
    ...
    def __deepcopy__(self, memo):
        temp_dict = deepcopy(self.to_dict(), memo)
        return Hparam(**temp_dict)
